...
switch($mode)
{
	...
	$val_artist = sql_select('disco_artists','artist_id = '.$val_album['artist_id'],'','','artist');
		...
		switch($val_album['type'])
		{
			...
			Case 'le single':
				...
				break;
			Case 'la compilation':
				$entete = '<a href="goldman_albums_compil.html"><b>'.ucfirst($lang['Compil_albums']).'</b></a> <b>></b> ';
				break;
		} $entete .= '<a href="goldman_album_'.$val_album['album_id'].'_' . url_title($val_album['title']) . '.html"><b>'.$val_album['title'].'</b></a> <b>></b> ';
		$entete .= '<a href="avis.php?mode=view_album&album_id='.$album_id.'&sid='.$_GET['sid'].'"><b>'.sprintf($lang['Votre'],$lang['avis']).'</b></a>';	

	$titre = $lang['votre_avis'].' '.$val_album['title'];
	...
	if (count($tab_avis)==0)
	$no_avis = sprintf($lang['no_avis'],$lang['cet'],$lang['album']);
	break;
	Case "view_song":
	$album_id = $_GET['album_id'];
	...
	if ($album_id != "")
	{
			...
			{
			Case 'l\'album':
				$entete = '<a href="goldman_albums-studio.html"><b>'.ucfirst($lang['Studio_albums']).'</b></a> <b>></b> ';
				break;
			Case 'le live':
				$entete = '<a href="goldman_albums_live.html"><b>'.ucfirst($lang['Live_albums']).'</b></a> <b>></b> ';
				break;
	  ...
	  $entete .= '<a href="avis.php?mode=goldman_chanson_'.$song_id . '_'.$album_id.'_' . url_title($val_song['title']) . '.html"><b>'.sprintf($lang['Votre'],$lang['avis']).'</b></a>';	

	$titre = sprintf($lang['Votre'],$lang['avis']).' '.$lang['sur'].' '.sprintf($lang['le'],$lang['Titre']).' '.$val_song['title'];
	...
	$tab_avis=select_liste($sql_avis);
	if (count($tab_avis)==0)
	$no_avis = sprintf($lang['no_avis'],$lang['ce'],$lang['titre']);
	break;
	Case "video":
	$album_id = $_GET['album_id'];
	...
	$val_artist = sql_select('disco_artists','artist_id = '.$val_album['artist_id'],'','','artist');
		...
		switch($val_album['type'])
		{
			...
			Case 'la compilation':
				...
				break;
			Case 'video':
				$entete = '<a href="goldman_albums_video.html"><b>'.$lang['Videothèque'].'</b></a> <b>></b> ';
				break;
		} $entete .= '<a href="goldman_album_'.$val_album['album_id'].'_' . url_title($val_album['title']) . '.html"><b>'.$val_album['title'].'</b></a> <b>></b> ';
		$entete .= '<a href=avis.php?mode=video&album_id='.$album_id.'&sid='.$_GET['sid'].'><b>'.sprintf($lang['Votre'],$lang['avis']).'</b></a>';	

	$titre = $lang['votre_avis_video'].' '.$val_album['title'];
	...
	$sql_avis = "SELECT 
		     ...
		     AND
		     		phpbb_forums.forum_id = phpbb_posts.forum_id
		     AND
		     		phpbb_forums.forum_name = 'DVD et VHS'
		     AND 
		     		phpbb_topics.topic_title LIKE '".addslashes($val_album['title'])."'
		     AND 
		     		phpbb_users.username <> 'Admins'
		     	";
	$tab_avis=select_liste($sql_avis);
	if (count($tab_avis)==0)
	$no_avis = sprintf($lang['no_avis'],$lang['cette'],$lang['vidéo']);
	break;
}
...
$template->assign_vars(array(
				...
				"L_RETOUR" => $lang['retour'],
				"ENTETE" => $entete,
				"MID" => $lang['Fichiers midi disponibles'],
				"NO_AVIS" => $no_avis,
			)
);
...
for ($i=0;$i<count($tab_avis);$i++)
{

	$template->assign_block_vars('switch_avis',array(
			'MESSAGE' => smilies_pass(bbencode_second_pass($tab_avis[$i]['post_text'],$tab_avis[$i]['bbcode_uid'])),
			'L_USERNAME' => $tab_avis[$i]['username'],
			'U_USERNAME' => append_sid($phpbb_root_path . 'forum/profile.php?mode=viewprofil&u='.$val_access['user_id']),
			)
		);
}
