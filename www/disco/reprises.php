...
// FIn Boris 27/02/2006
{
	...
	$sql_singer = "SELECT A.* FROM disco_artists A, disco_artists_job B WHERE A.artist_id = B.artist_id AND B.job = 'Interprète' AND project = ".$val_reprise['song_id'];
	$result_singer = mysql_query($sql_singer);
	$reprises = '<a href="goldman_chanson_'.$val_reprise['song_id'].'_' . url_title($val_reprise['song_id']).'.html"><b>'.$val_reprise['title'].'</b></a> <i>('.$val_language['language'].')</i><br />'.$lang['Interprété'].' '.$lang['par'].' ';
	$list_singer .= "";
	While ($val_singer = mysql_fetch_array($result_singer))
	{
		/*
		// Gestion Multi-artistes obsolète
