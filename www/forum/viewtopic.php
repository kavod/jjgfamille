...
if (strpos($HTTP_GET_VARS['highlight'],'%27')!==false)
{
	...
	if ( $HTTP_GET_VARS['view'] == 'newest' )
	{
		if ( isset($HTTP_COOKIE_VARS[$board_config['cookie_name'] . '_sid']) || isset($HTTP_GET_VARS['sid']) )
		{
			...
			if ( $session_id )
			{
				...
				{
					redirect("forum/viewtopic.$phpEx?sid=$session_id&" . POST_POST_URL . "=$post_id#$post_id");
				}
				else
				{
					redirect("forum/viewtopic.$phpEx?" . POST_POST_URL . "=$post_id#$post_id");
				}
			}
		}
	...
	else if ( $HTTP_GET_VARS['view'] == 'next' || $HTTP_GET_VARS['view'] == 'previous' )
	{
		...
		$sql = "SELECT t.topic_id
			...
			WHERE
				...
				AND t.topic_moved_id = 0 AND t.topic_last_post_id $sql_condition t2.topic_last_post_id
...
if( $userdata['session_logged_in'] )
{
	...
	else
	{
		if ( isset($HTTP_GET_VARS['watch']) )
		{
			...
			$template->assign_vars(array(
				'META' => '<meta http-equiv="refresh" content="3;url=' . append_sid($phpbb_root_path . "forum/viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;start=$start") . '">')
			);

			$message = $lang['You_are_watching'] . '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . append_sid($phpbb_root_path . "forum/viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;start=$start") . '">', '</a>');
			message_die(GENERAL_MESSAGE, $message);
		}
		else
		{
...
else
{
	if ( isset($HTTP_GET_VARS['unwatch']) )
	{
		if ( $HTTP_GET_VARS['unwatch'] == 'topic' )
		{
			redirect(append_sid("forum/login.$phpEx?redirect=forum/viewtopic.$phpEx&" . POST_TOPIC_URL . "=$topic_id&unwatch=topic", true));
		}
	}
	else
	{
...
// templating vars
//
$printer_topic_url = append_sid($phpbb_root_path . "forum/viewtopic.$phpEx?printertopic=1&" . POST_TOPIC_URL . "=$topic_id&start=$start&postdays=$post_days&postorder=$post_order&vote=viewresult");
$new_topic_url = append_sid($phpbb_root_path . "forum/posting.$phpEx?mode=newtopic&amp;" . POST_FORUM_URL . "=$forum_id");
$reply_topic_url = append_sid($phpbb_root_path . "forum/posting.$phpEx?mode=reply&amp;" . POST_TOPIC_URL . "=$topic_id");
$view_forum_url = append_sid($phpbb_root_path . "forum/viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id");
$view_prev_topic_url = append_sid($phpbb_root_path . "forum/viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;view=previous");
$view_next_topic_url = append_sid($phpbb_root_path . "forum/viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;view=next");
...
if ( $can_watch_topic )
{
	...
	{
		$s_watching_topic = "<a href=\"" . $phpbb_root_path . "forum/viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;unwatch=topic&amp;start=$start&amp;sid=" . $userdata['session_id'] . '">' . $lang['Stop_watching_topic'] . '</a>';
...
$pagination = generate_pagination($phpbb_root_path . "forum/viewtopic.$phpEx?". $pagination_printertopic . POST_TOPIC_URL . "=$topic_id&amp;postdays=$post_days&amp;postorder=$post_order&amp;". $pagination_highlight . $pagination_finish_rel, $total_replies, $pagination_ppp, $start,true,$topic_title);
if($pagination != '' && isset($pagination_printertopic))
{
	$pagination .= " &nbsp;<a href=\"" . $phpbb_root_path . "forum/viewtopic.$phpEx?". $pagination_printertopic. POST_TOPIC_URL . "=$topic_id&amp;postdays=$post_days&amp;postorder=$post_order&amp;". $pagination_highlight. "start=0&amp;finish_rel=-10000\" title=\"" . $lang['printertopic_cancel_pagination_desc'] . "\">:|&nbsp;|:</a>";
}
...
if ( !empty($forum_topic_data['topic_vote']) )
{
	...
	// Boris 12/02/2007 Vote Manage Mod : add vd.vote_max, vd.vote_voted, vd.vote_hide, vd.vote_undo
	...
	if ( $vote_info = $db->sql_fetchrowset($result) )
	{
		...
		//$sql = "SELECT vote_id
		// END OF DELETION
		// changing vote_id by vote_option_id
		...
		if ( $user_voted || $view_result || $poll_expired || !$is_auth['auth_vote'] || $forum_topic_data['topic_status'] == TOPIC_LOCKED )
		{
			...
			}	// Boris 12/02/2007 Vote Manage Mod INSERTION
		...
		else
		{
			...
			for($i = 0; $i < $vote_options; $i++) // ORIGINAL FROM INSERTION
			{
				$new_sort[$vote_info[$i]['vote_option_id']] = $i;
			}
			ksort($new_sort);
