...
if ( isset($HTTP_POST_VARS['groupstatus']) && $group_id )
{
	...
	{
		redirect(append_sid("forum/login.$phpEx?redirect=forum/groupcp.$phpEx&" . POST_GROUPS_URL . "=$group_id", true));
	}
...
else if ( isset($HTTP_POST_VARS['unsub']) || isset($HTTP_POST_VARS['unsubpending']) && $group_id )
{
	...
	if ( $cancel )
	{
		redirect(append_sid("forum/groupcp.$phpEx", true));
	}
		...
		case 'oracle':
			$sql = "SELECT g.group_moderator, g.group_type, aa.auth_mod 
				...
				WHERE g.group_id = $group_id
					AND aa.group_id (+) = g.group_id
				ORDER BY aa.auth_mod DESC";
//					AND aa.group_id (+) = g.group_id"; // UPDATE 2.0.22
			break;
		...
		default:
			$sql = "SELECT g.group_moderator, g.group_type, aa.auth_mod 
				...
				WHERE g.group_id = $group_id
				ORDER BY aa.auth_mod DESC";
//				WHERE g.group_id = $group_id"; // Update 2.0.22
			break;
	}
	if ( !($result = $db->sql_query($sql)) )
	{
