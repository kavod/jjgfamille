...
if ( $cancel )
{
	redirect(append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=$folder", true));
}
...
$start = ($start < 0) ? 0 : $start; // Boris 11/11/2007 // Update 2.0.22
...
//
$inbox_img = ( $folder != 'inbox' || $mode != '' ) ? '<a href="' . append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=inbox") . '"><img src="' . $images['pm_inbox'] . '" border="0" alt="' . $lang['Inbox'] . '" /></a>' : '<img src="' . $images['pm_inbox'] . '" border="0" alt="' . $lang['Inbox'] . '" />';
$inbox_url = ( $folder != 'inbox' || $mode != '' ) ? '<a href="' . append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=inbox") . '">' . $lang['Inbox'] . '</a>' : $lang['Inbox'];

$outbox_img = ( $folder != 'outbox' || $mode != '' ) ? '<a href="' . append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=outbox") . '"><img src="' . $images['pm_outbox'] . '" border="0" alt="' . $lang['Outbox'] . '" /></a>' : '<img src="' . $images['pm_outbox'] . '" border="0" alt="' . $lang['Outbox'] . '" />';
$outbox_url = ( $folder != 'outbox' || $mode != '' ) ? '<a href="' . append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=outbox") . '">' . $lang['Outbox'] . '</a>' : $lang['Outbox'];

$sentbox_img = ( $folder != 'sentbox' || $mode != '' ) ? '<a href="' . append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=sentbox") . '"><img src="' . $images['pm_sentbox'] . '" border="0" alt="' . $lang['Sentbox'] . '" /></a>' : '<img src="' . $images['pm_sentbox'] . '" border="0" alt="' . $lang['Sentbox'] . '" />';
$sentbox_url = ( $folder != 'sentbox' || $mode != '' ) ? '<a href="' . append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=sentbox") . '">' . $lang['Sentbox'] . '</a>' : $lang['Sentbox'];

$savebox_img = ( $folder != 'savebox' || $mode != '' ) ? '<a href="' . append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=savebox") . '"><img src="' . $images['pm_savebox'] . '" border="0" alt="' . $lang['Savebox'] . '" /></a>' : '<img src="' . $images['pm_savebox'] . '" border="0" alt="' . $lang['Savebox'] . '" />';
$savebox_url = ( $folder != 'savebox' || $mode != '' ) ? '<a href="' . append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=savebox") . '">' . $lang['Savebox'] . '</a>' : $lang['Savebox'];
...
if ( $mode == 'newpm' )
{
	...
	if ( $userdata['session_logged_in'] )
	{
		...
		$l_new_message .= '<br /><br />' . sprintf($lang['Click_view_privmsg'], '<a href="' . append_sid($phpbb_root_path . "forum/privmsg.".$phpEx."?folder=inbox") . '" onclick="jump_to_inbox();return false;" target="_new">', '</a>');
	}
	else
	{
...
else if ( $mode == 'read' )
{
	...
	if ( !$userdata['session_logged_in'] )
	{
		redirect(append_sid($phpbb_root_path . "forum/login.$phpEx?redirect=forum/privmsg.$phpEx&folder=$folder&mode=$mode&" . POST_POST_URL . "=$privmsgs_id", true));
	}
	...
	//
	$post_urls = array(
		'post' => append_sid($phpbb_root_path . "forum/privmsg.$phpEx?mode=post"),
		'reply' => append_sid($phpbb_root_path . "forum/privmsg.$phpEx?mode=reply&amp;" . POST_POST_URL . "=$privmsg_id"),
		'quote' => append_sid($phpbb_root_path . "forum/privmsg.$phpEx?mode=quote&amp;" . POST_POST_URL . "=$privmsg_id"),
		'edit' => append_sid($phpbb_root_path . "forum/privmsg.$phpEx?mode=edit&amp;" . POST_POST_URL . "=$privmsg_id")
	);
	$post_icons = array(
	...
	$template->assign_vars(array(
		...
		'S_PRIVMSGS_ACTION' => append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=$folder"),
	...
	$temp_url = append_sid($phpbb_root_path . "forum/profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $user_id_from);
	$profile_img = '<a href="' . $temp_url . '"><img src="' . $images['icon_profile'] . '" alt="' . $lang['Read_profile'] . '" title="' . $lang['Read_profile'] . '" border="0" /></a>';
	$profile = '<a href="' . $temp_url . '">' . $lang['Read_profile'] . '</a>';

	$temp_url = append_sid($phpbb_root_path . "forum/privmsg.$phpEx?mode=post&amp;" . POST_USERS_URL . "=$user_id_from");
	$pm_img = '<a href="' . $temp_url . '"><img src="' . $images['icon_pm'] . '" alt="' . $lang['Send_private_message'] . '" title="' . $lang['Send_private_message'] . '" border="0" /></a>';
	$pm = '<a href="' . $temp_url . '">' . $lang['Send_private_message'] . '</a>';

	if ( !empty($privmsg['user_viewemail']) || $userdata['user_level'] == ADMIN )
	{
		$email_uri = ( $board_config['board_email_form'] ) ? append_sid($phpbb_root_path . "forum/profile.$phpEx?mode=email&amp;" . POST_USERS_URL .'=' . $user_id_from) : 'mailto:' . $privmsg['user_email'];
	...
	$temp_url = append_sid($phpbb_root_path . "forum/search.$phpEx?search_author=" . urlencode($username_from) . "&amp;showresults=posts");
...
else if ( ( $delete && $mark_list ) || $delete_all )
{
	if ( !$userdata['session_logged_in'] )
	{
		redirect(append_sid($phpbb_root_path . "forum/login.$phpEx?redirect=forum/privmsg.$phpEx&folder=inbox", true));
	}
	...
	if ( !$confirm )
	{
		...
		$template->assign_vars(array(
			...
			'S_CONFIRM_ACTION' => append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=$folder"),
			'S_HIDDEN_FIELDS' => $s_hidden_fields)
		);
...
else if ( $save && $mark_list && $folder != 'savebox' && $folder != 'outbox' )
{
	...
	if (sizeof($mark_list))
	{
		...
		redirect(append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=savebox", true));
	}
}
else if ( $submit || $refresh || $mode != '' )
{
	if ( !$userdata['session_logged_in'] )
	{
		...
		redirect(append_sid($phpbb_root_path . "forum/login.$phpEx?redirect=forum/privmsg.$phpEx&folder=$folder&mode=$mode" . $user_id, true));
	}
...
// End Update
	...
	if ( $submit && !$error )
	{
		...
		// Boris 13/05/2006
		// Mesures anti-spam
		check_suffisant_pm_credit($to_userdata);
		// fin de modif
		...
		$template->assign_vars(array(
			'META' => '<meta http-equiv="refresh" content="3;url=' . append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=inbox") . '">')
		);

		$msg = $lang['Message_sent'] . '<br /><br />' . sprintf($lang['Click_return_inbox'], '<a href="' . append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=inbox") . '">', '</a> ') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid($phpbb_root_path . "forum/index.$phpEx") . '">', '</a>');
	...
	else if ( $preview || $refresh || $error )
	{
		...
		// $privmsg_message = preg_replace('#<textarea>#si', '&lt;textarea&gt;', $privmsg_message); // Boris 11/11/2007 // Update 2.0.22
		if ( !$preview )
		{
	...
	if ($error)
	{
		$privmsg_message = htmlspecialchars($privmsg_message); // Boris 11/11/2007 // Update 2.0.22
		$template->set_filenames(array(
	...
	$s_hidden_fields = '<input type="hidden" name="folder" value="' . $folder . '" />';
	$s_hidden_fields .= '<input type="hidden" name="mode" value="' . $mode . '" />';
	$s_hidden_fields .= '<input type="hidden" name="sid" value="' . $userdata['session_id'] . '" />'; // Boris 11/11/2007 // Update 2.0.22
	if ( $mode == 'edit' )
	{
		$s_hidden_fields .= '<input type="hidden" name="' . POST_POST_URL . '" value="' . $privmsg_id . '" />';
	}
	...
	*/
	...
	// Boris 13/05/2006
	// Mesures anti-spam
	$val_rank = select_element("SELECT MIN(rank_min) `rank_min` FROM `" . RANKS_TABLE . "` WHERE `rank_special` = '0'",'Erreur SQL.<br />Merci de prévenir le support technique.',true);
	$PM_credit = ($userdata['user_posts'] >= $val_rank['rank_min']) ? $lang['unlimited'] : sprintf($lang['PM_nb_msg'],$userdata['user_private_posts']);
	$PM_credit .= sprintf(' [ <a href="%s">%s</a> ]',append_sid($phpbb_root_path . 'forum/faq.php?mode=faq').'#33',$lang['Know_more']);
	// fin modif
	...
	$template->assign_vars(array(
		...
		'BBCODE_STATUS' => sprintf($bbcode_status, '<a href="' . append_sid($phpbb_root_path . "forum/faq.$phpEx?mode=bbcode") . '" target="_phpbbcode">', '</a>'), 
		...
		// Boris 13/05/2006
		// Mesures anti-spam
		'CREDIT' => sprintf($lang['PM_credit'],$PM_credit),
		// fin modif
		...
		'S_POST_ACTION' => append_sid($phpbb_root_path . "forum/privmsg.$phpEx"),
			
		'U_SEARCH_USER' => append_sid($phpbb_root_path . "forum/search.$phpEx?mode=searchuser"), 
		'U_VIEW_FORUM' => append_sid($phpbb_root_path . "forum/privmsg.$phpEx"))
	);
...
// New message
//
$post_new_mesg_url = '<a href="' . append_sid($phpbb_root_path . "forum/privmsg.$phpEx?mode=post") . '"><img src="' . $images['post_new'] . '" alt="' . $lang['Send_a_new_message'] . '" border="0" /></a>';
...
$post_pm = append_sid($phpbb_root_path . "forum/privmsg.$phpEx?mode=post");
...
$template->assign_vars(array(
	...
	'U_POST_NEW_TOPIC' => append_sid($phpbb_root_path . "forum/privmsg.$phpEx?mode=post"))
);
...
if ( $row = $db->sql_fetchrow($result) )
{
	...
	do
	{
		...
		$u_from_user_profile = append_sid($phpbb_root_path . "forum/profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$msg_userid");
		...
		$u_subject = append_sid($phpbb_root_path . "forum/privmsg.$phpEx?folder=$folder&amp;mode=read&amp;" . POST_POST_URL . "=$privmsg_id");
