...
switch( $mode )
{
	case 'delete':
		...
		if ( $confirm )
		{
			...
			if ( !empty($topic_id) )
			{
				$redirect_page = $phpbb_root_path . "forum/viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'];
				$l_redirect = sprintf($lang['Click_return_forum'], '<a href="' . $redirect_page . '">', '</a>');
			}
			else
			{
				$redirect_page = $phpbb_root_path . "forum/modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'];
	...
	case 'move':
		...
		if ( $confirm )
		{
			...
			$val_from_forum = select_element("SELECT forum_name FROM " . FORUMS_TABLE . " WHERE forum_id = " .  $old_forum_id,true,'forum introuvable');
			...
			if ( !empty($topic_id) )
			{
				$redirect_page = $phpbb_root_path . "forum/viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'];
	...
	case 'lock':
		...
		if ( !empty($topic_id) )
		{
			$redirect_page = "forum/viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'];
			$message = sprintf($lang['Click_return_topic'], '<a href="' . $redirect_page . '">', '</a>');
		}
		else
		{
			$redirect_page = "forum/modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'];
			$message = sprintf($lang['Click_return_modcp'], '<a href="' . $redirect_page . '">', '</a>');
		}
		...
		else
		{
			...
			if( ( $total_posts = $db->sql_numrows($result) ) > 0 )
			{
				...
				$template->assign_vars(array(
					...
					'U_VIEW_FORUM' => append_sid($phpbb_root_path . "forum/viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id"), 
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			do
			{
				...
				$template->assign_block_vars('userrow', array(
					...
					'U_SEARCHPOSTS' => append_sid($phpbb_root_path . "forum/search.$phpEx?search_author=" . (($id == ANONYMOUS) ? 'Anonymous' : urlencode($username)) . "&amp;showresults=topics"))
				);
