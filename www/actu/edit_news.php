...
include($phpbb_root_path . 'functions/functions_xml.php');
...
if ($_GET['mode'] == 'supp')
{
	if (!isset($_GET['news_id']) || $_GET['news_id'] == '')
		message_die(GENERAL_MESSAGE,'Variable news_id vide');
	else $news_id = $_GET['news_id'];
	
	$sql = "DELETE FROM famille_news WHERE news_id = '" . $news_id . "'";
	mysql_query($sql) or message_die(CRITICAL_ERROR,'Erreur Interne<br />' . mysql_error());
	logger('Suppression de la news N°' . $news_id);
	genere_rss();
	
	
	$template->assign_vars(array(
			'META' => '<meta http-equiv="refresh" content="3;url=' . append_sid($phpbb_root_path . "actu/news." . $phpEx) . '">')
		);
		$message .=  '<br /><br />' . sprintf($lang['Supp_news_ok'], '<a href="' . append_sid($phpbb_root_path . "actu/news." . $phpEx) . '">', '</a>');
...
if ($_GET['mode'] == 'edit')
{
	...
	$val_news = select_element("SELECT * FROM famille_news WHERE news_id = '" . $news_id . "'",true,'News introuvable');
	
	$texte = htmlentities(str_replace("<br />","",preg_replace('/\:(([a-z0-9]:)?)' . $val_news['bbcode_uid'] . '/s', '', $val_news['news'])));
	$title = $val_news['title'];
	//$date = mktime(12,0,0,substr($val_news['date_hot'],4,2),substr($val_news['date_hot'],6,2),substr($val_news['date_hot'],0,4));
	$date_hot = date('d/m/Y',$val_news['date_hot_unix']);
}

if ($_GET['mode'] == 'doedit' || $_GET['mode'] == 'doadd')
{
	if ($_GET['mode'] == 'doedit' && (!isset($_GET['news_id']) || $_GET['news_id'] == ''))
		message_die(GENERAL_MESSAGE,'Variable news_id vide');
	else $news_id = $_GET['news_id'];
	...
	if (!$error)
	{
		...
		$news_bb = bbencode_first_pass($texte, $bbcode_uid);
		
		if ($_GET['mode'] == 'doedit')
		{
			$sql = "UPDATE famille_news SET title = '$title', news = '$news_bb', date_hot_unix = '$date_bdd', bbcode_uid = '$bbcode_uid' WHERE news_id = '$news_id'";
			logger('Modification de la news "' . $title .'"');
		} else
		{
			$sql = "INSERT INTO famille_news (date_unix,user_id,title,news,artist_id,bbcode_uid,date_hot_unix) 
				VALUES ('" . date('U') . "','" . $userdata['user_id'] . "','$title','$news_bb',1,'$bbcode_uid','$date_bdd')";
			logger('Ajout de la news "' . $title . '"');
		}
		mysql_query($sql) or message_die(CRITICAL_ERROR,'Erreur d\'écriture dans la base de données');
		genere_rss();
		
		$template->assign_vars(array(
			'META' => '<meta http-equiv="refresh" content="3;url=' . append_sid($phpbb_root_path . "actu/news." . $phpEx . '?news_id=' . $news_id) . '">')
		);
		$message .=  '<br /><br />' . sprintf($lang['Change_news_ok'], '<a href="' . append_sid($phpbb_root_path . "actu/news." . $phpEx . '?news_id=' . $news_id) . '">', '</a>');
		message_die(GENERAL_MESSAGE, $message);
...
if ($_GET['mode'] == 'edit')
{
	$template->assign_vars(array(
				...
				'TEXTE_VALUE' => $texte,
				'HOT_VALUE' => $date_hot,
				'ACTION' => $lang['Edit_news'],
				
				'U_ACTION' => append_sid($phpbb_root_path . 'actu/edit_news.php?mode=doedit&news_id=' . $news_id),
				'L_SUBMIT' => $lang['Modifier'],
				'L_SUPP' => $lang['Supprimer_news'],
				'L_CONFIRM_SUPP' => str_replace("'","\'",$lang['confirm_supp_news']),
				'U_SUPP' => append_sid($phpbb_root_path . 'actu/edit_news.php?mode=supp&news_id=' . $news_id),
...
if ($_GET['mode'] == 'add')
{
	$template->assign_vars(array(
				...
				'TEXTE_VALUE' => $texte,
				'HOT_VALUE' => $date_hot,
				'ACTION' => $lang['Add_news'],
				
				'U_ACTION' => append_sid($phpbb_root_path . 'actu/edit_news.php?mode=doadd'),
...
$template->assign_vars(array(
				...
				"U_RETOUR" => append_sid($phpbb_root_path . 'actu/news.php'),
