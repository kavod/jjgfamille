...
function user_avatar_gallery($mode, &$error, &$error_msg, $avatar_filename)
{
	...
	if ( file_exists(@phpbb_realpath($phpbb_root_path . $board_config['avatar_gallery_path'] . '/' . $avatar_filename)) && ($mode == 'editprofile') )
	{
...
function user_avatar_upload($mode, $avatar_mode, &$current_avatar, &$current_type, &$error, &$error_msg, $avatar_filename, $avatar_realname, $avatar_filesize, $avatar_filetype)
{
	...
	if ( $width > 0 && $height > 0 && $width <= $board_config['avatar_max_width'] && $height <= $board_config['avatar_max_height'] )
	{
		...
		{
			/*update 2.0.18
			if ( file_exists(@phpbb_realpath($phpbb_root_path . $board_config['avatar_path'] . '/' . $current_avatar)) )
			{
				@unlink($phpbb_root_path . $board_config['avatar_path'] . '/' . $current_avatar);
			}*/
...
function user_photo_upload($user_id, $photo_mode, &$error, &$error_msg, $photo_filename, $photo_realname, $photo_filesize, $photo_filetype)
{
	global $board_config, $user_ip, $db, $lang;

	$ini_val = ( @phpversion() >= '4.0.0' ) ? 'ini_get' : 'get_cfg_var';

	if ( file_exists($photo_filename) && preg_match('/\.(jpg|gif|png)$/i', $photo_realname) )
	{
		if ( $photo_filesize <= (190*1024) && $photo_filesize > 0 )
		{
	...
	if ( !($imgtype = check_image_type($photo_filetype, $error, $error_msg)) )
	{
		return;
	}
	...
	if ( $width <= 200 && $height <= 400 )
	{
		...
		@unlink('../images/forum/photos/photo_' . $user_id.".jpg");
		@unlink('../images/forum/photos/photo_' . $user_id.".gif");
		...
		$move_file($photo_filename, "../images/forum/photos/photo_".$user_id.$imgtype);
		$photo_sql = ", photo = 'Y' ";

		@chmod($phpbb_root_path . "images/forum/photos/photo_".$user_id.$imgtype, 0777);

	}
	else
	{
		$l_photo_size = sprintf($lang['Photo_imagesize'], 200, 400);
