...
function auth($type, $forum_id, $userdata, $f_access = '')
{
	...
	if ( empty($f_access) )
	{
		...
		if ($forum_id > -1)
		{
			...
			$sql = "SELECT a.forum_id, $a_sql
				FROM " . FORUMS_TABLE . " a
				$forum_match_sql";
			$auth_table = AUTH_ACCESS_TABLE;
		}
		else
		{
			$forum_match_sql = ( $forum_id != AUTH_LIST_ALL ) ? "WHERE a.cate_id = " . (0-$forum_id) : '';
			$sql = "SELECT a.cate_id forum_id, $a_sql
				FROM blogs_cate a
				$forum_match_sql";
			$auth_table = 'blogs_auth';
		}
		// fin Boris 12/09/2006
	...
	// Boris 12/09/2006 / updated 08/10/2006
	// Ajout gestion blog
	// added
	if ($forum_id > -1)
	{
	...
	{
		$auth_table = 'blogs_auth';
		$forum_match_sql = "AND a.`forum_id` = '" . (0-$forum_id) ."'";
	}
	//$auth_table = ($forum_id > -1) ? AUTH_ACCESS_TABLE : 'blogs_auth';
	// fin Boris 12/09/2006
	...
	if ( $userdata['session_logged_in'] )
	{
		...
		$sql = "SELECT a.forum_id, $a_sql, a.auth_mod 
			FROM $auth_table a, " . USER_GROUP_TABLE . " ug 
	...
	for($i = 0; $i < count($auth_fields); $i++)
	{
		...
		if ( $forum_id != AUTH_LIST_ALL )
		{
			...
			switch( $value )
			{
				...
				case AUTH_ACL:
					// !! ne pas utiliser dans les blogs
