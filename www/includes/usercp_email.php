...
{
	redirect(append_sid("forum/login.$phpEx?redirect=forum/profile.$phpEx&mode=email&" . POST_USERS_URL . "=$user_id", true));
}

$sql = "SELECT user_id, username, user_email, user_viewemail, user_lang, user_level  
	FROM " . USERS_TABLE . " 
...
// End Update 2.0.22
		...
		if ( $row['user_viewemail'] || $userdata['user_level'] == ADMIN )
		{
			...
			$PM_credit = ($user_id=='1' || is_responsable($user_id) || $userdata['user_posts'] >= $val_rank['rank_min']) ? $lang['unlimited'] : sprintf($lang['PM_nb_msg'],$userdata['user_private_posts']);
