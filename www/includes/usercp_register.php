	...
	$mode == 'register' )
{
	...
	// update 2.0.19
	//$strip_var_list = array('username' => 'username', 'email' => 'email', 'icq' => 'icq', 'aim' => 'aim', 'msn' => 'msn', 'yim' => 'yim', 'website' => 'website', 'location' => 'location', 'occupation' => 'occupation', 'interests' => 'interests', 'birth_day' => 'birth_day', 'birth_month' => 'birth_month', 'birth_year' => 'birth_year');
	//$strip_var_list['confirm_code'] = 'confirm_code';
	$strip_var_list = array('email' => 'email', 'icq' => 'icq', 'aim' => 'aim', 'msn' => 'msn', 'yim' => 'yim', 'website' => 'website', 'location' => 'location', 'occupation' => 'occupation', 'interests' => 'interests', 'confirm_code' => 'confirm_code', 'abq_aw' => $abq_config['postvariablename'], 'idabq' => 'idabq', 'birth_day' => 'birth_day', 'birth_month' => 'birth_month', 'birth_year' => 'birth_year');
	...
	validate_optional_fields($icq, $aim, $msn, $yim, $website, $location, $occupation, $interests, $signature, $birth_day, $birth_month, $birth_year);
	...
	// Boris 11/11/2007
	// Update 2.0.22
	$sid = (isset($HTTP_POST_VARS['sid'])) ? $HTTP_POST_VARS['sid'] : 0; // Add 2.0.22
	// End Update 2.0.22
...
// End 2.0.22
	...
	if ($board_config['enable_confirm'] && $mode == 'register')
	{
		...
		else
		{
			...
			if (!($result = $db->sql_query($sql)))
			{
				// Boris 11/11/2007
				// Update 2.0.22
				// message_die(GENERAL_ERROR, 'Could not obtain confirmation code', __LINE__, __FILE__, $sql); // Del 2.0.22
				message_die(GENERAL_ERROR, 'Could not obtain confirmation code', '', __LINE__, __FILE__, $sql); // Add 2.0.22
				// End Update 2.0.22
			}
				...
				*/
				...
				else
				{
					...
					if (!$db->sql_query($sql))
					{
						// Boris 11/11/2007
						// Update 2.0.22
						//message_die(GENERAL_ERROR, 'Could not delete confirmation code', __LINE__, __FILE__, $sql); // Del 2.0.22
						message_die(GENERAL_ERROR, 'Could not delete confirmation code', '', __LINE__, __FILE__, $sql); // Add 2.0.22
						// End Update 2.0.22
					}
				}
			}
	...
	{
		$img_url = $phpbb_root_path . "images/forum/photos/photo_".$user_id;
		$ext = find_image($img_url);
		unlink($img_url . '.' . $ext);
		/*if (!unlink("../images/forum/photos/photo_".$user_id.".gif"))
			if (unlink("../images/forum/photos/photo_".$user_id.".jpg"))
				unlink("../images/forum/photos/photo_".$user_id.".png");*/
		$photo_sql = ", photo = 'N' ";
	}
	else if (  !empty($user_photo_upload) || !empty($user_photo_name)  )
	{
		if ( !empty($user_photo_upload) )
		{
			$photo_mode = ( !empty($user_avatar_name) ) ? 'local' : 'remote';
			$photo_sql = user_photo_upload( $userdata['user_id'],$photo_mode, $error, $error_msg, $user_photo_upload, $user_photo_name, $user_photo_size, $user_photo_filetype);
		}
		else if ( !empty($user_photo_name) )
		{
			$l_photo_size = sprintf($lang['Photo_filesize'], 190);
	...
	/*if ( $mode == 'editprofile' && $userdata['confirm_assoc']=='N' && (!isset($_POST['confirm_assoc']) || $_POST['confirm_assoc'] == ''))
	{
		$error = true;
		$error_msg = 'Vous devez accepter le changement de propri&eacute;taire du fichier des inscription pour enregistrer votre profil';
	}*/
	...
	if ( !$error )
	{
		...
		if ( $mode == 'editprofile' )
		{
			...
			//$confirm_assoc = (isset($_POST['confirm_assoc']) && $_POST['confirm_assoc']=='ok') ? "Y" : "N";
			$sql = "UPDATE " . USERS_TABLE . "
				SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) ."', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_sig_bbcode_uid = '$signature_bbcode_uid', user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_attachsig = $attachsig, user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_notify_pm = $notifypm, user_popup_pm = $popup_pm, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_active = $user_active, user_actkey = '" . str_replace("\'", "''", $user_actkey) . "'" . $avatar_sql 
				 . ", birth_day = " . $birth_day . ", birth_month = " . $birth_month . ", birth_year = " . $birth_year . $photo_sql;
			/*if (isset($_POST['confirm_assoc'])) 
				$sql .= ", confirm_assoc = '$confirm_assoc' ";*/
		...
		else
		{
			...
			$sql = "INSERT INTO " . USERS_TABLE . "	(
					...
					birth_year)";
					//confirm_assoc)
			$sql .= " VALUES ($user_id, '" . str_replace("\'", "''", $username) . "', " . time() . ", '" . str_replace("\'", "''", $new_password) . "', '" . str_replace("\'", "''", $email) . "', '" . str_replace("\'", "''", $icq) . "', '" . str_replace("\'", "''", $website) . "', '" . str_replace("\'", "''", $occupation) . "', '" . str_replace("\'", "''", $location) . "', '" . str_replace("\'", "''", $interests) . "', '" . str_replace("\'", "''", $signature) . "', '$signature_bbcode_uid', $avatar_sql, $viewemail, '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', '" . str_replace("\'", "''", $yim) . "', '" . str_replace("\'", "''", $msn) . "', $attachsig, $allowsmilies, $allowhtml, $allowbbcode, $allowviewonline, $notifyreply, $notifypm, $popup_pm, $user_timezone, '" . str_replace("\'", "''", $user_dateformat) . "', '" . str_replace("\'", "''", $user_lang) . "', $user_style, 0, 1,   ";
			if ( $board_config['require_activation'] == USER_ACTIVATION_SELF || $board_config['require_activation'] == USER_ACTIVATION_ADMIN || $coppa )
			{
				...
				$key_len = ( $key_len > 6 ) ? $key_len : 6;
				$user_actkey = substr($user_actkey, 0, $key_len);
				$sql .= "0, '" . str_replace("\'", "''", $user_actkey) .'\', ' . $birth_day . ",". $birth_month . "," . $birth_year  . ",'Y')";
			}
			else
			{
				$sql .= "1, ''," . $birth_day . ",". $birth_month . "," . $birth_year /*. ",'Y'"*/ . ")";
			}
...
else if ( $mode == 'editprofile' && !isset($HTTP_POST_VARS['avatargallery']) && !isset($HTTP_POST_VARS['submitavatar']) && !isset($HTTP_POST_VARS['cancelavatar']) )
{
	...
	if ($userdata['birth_day']!=0) $birth_day = $userdata['birth_day'];
	if ($userdata['birth_month']!=0) $birth_month = $userdata['birth_month'];
	if ($userdata['birth_year']!=1000) $birth_year = $userdata['birth_year'];
...
else
{
	...
	if ( $mode == 'editprofile' )
	{
		...
		/*if ($userdata['confirm_assoc'] == 'N')
		{
			$template->assign_block_vars('confirm_assoc', array(
								'L_CONFIRM_ASSOC' => '<b>IMPORTANT !!!</b> JJG famille est maintenant r&eacute;gie par l\'association <i>famille Musik</i>. Pour des raisons l&eacute;gales, vous devez confirmer que vous acceptez que l\'association soit le nouveau propri&eacute;taire des informations personnelles que vous remplissez ci-dessous.<br />
								<small>Tout comme pr&eacute;c&eacute;dement, nous nous engageons &agrave; ne conserver ces information que pour le fonctionnement du site et que ces derni&egrave;res ne seront communiqu&eacute;es &agrave; personne</small>',
								'YES_CONFIRM_ASSOC' => sprintf('Oui, j\'accepte que <i>famille Musik</i> devienne propri&eacute;taire des informations de mon profil. Ces informations sont enregistr&eacute;s dans un fichier enregistr&eacute; par la d&eacute;claration N&deg;%s de la CNIL.',$lang['n_cnil'])
							));
		}*/
	}
	// Anti Bot Question Mod - Start
	...
	if (!empty($board_config['enable_confirm']) && $mode == 'register')
	{
		...
		*/
		...
		$confirm_image = '<img src="' . append_sid($phpbb_root_path . "forum/profile.$phpEx?mode=confirm&amp;id=$confirm_id") . '" alt="" title="" />';
		// fin update 2.0.21
	...
	// Photo part
	$photo_url = $phpbb_root_path . $board_config['photo_path'].'photo_'.$userdata['user_id'];
	if (is_file($photo_url.'.png'))
	{
		$photo = $photo_url.'.png';
	} else
		if (is_file($photo_url.'.jpg'))
			$photo = $photo_url.'.jpg';
		else $photo = (is_file($photo_url.'.gif')) ? $photo_url.'.gif' : '';
	$photo = ($photo!='') ? '<img src="'.$photo.'" border="0">' : '';
	...
	$template->assign_vars(array(
		...
		*/
		...
		'BIRTHDAY_SELECT' => birthday_select(array($birth_day,$birth_month,$birth_year),'birth'),
		...
		'L_PHOTO_EXPLAIN' => sprintf($lang['Photo_explain'],(round($board_config['avatar_filesize'] / 1024))),
