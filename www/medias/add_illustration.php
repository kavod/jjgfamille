...
if ($_GET['mode'] == 'add')
{
	...
	if (!$error)
	{
		$sql_update = "INSERT INTO media_illustrations (emission_id,user_id,username,description,photographe) VALUES ( '".$emission_id."' , '".$userdata['user_id']."' , '".$userdata['username']."', '".$comment."' , '".$photographe."')";
		mysql_query($sql_update) or list($error,$error_msg) = array( true , "Erreur durant l'insertion de la base de données<br />".$sql_update);
		
		
		$illustration_id = mysql_insert_id();
		...
		if (!$error && $user_upload!= '')
		{
			user_upload_easy(
					...
					$phpbb_root_path . 'images/medias/illustration_' . $emission_id .'_'.$illustration_id ,
					array(
			...
			if ($error)
			{
				$sql_del = "DELETE FROM  media_illustrations WHERE illustration_id = " . $illustration_id;
		...
		if (!$error)
		{
			logger("Ajout de l'illustration de l'emission $emission_id dans la médiathèque");
			$template->assign_vars(array(
			'META' => '<meta http-equiv="refresh" content="3;url=' . append_sid("view_emission." . $phpEx."?emission_id=".$emission_id) . '">')
			);
			$message =  sprintf($lang['Upload_illustration_ok'], '<a href="' . append_sid("view_emission." . $phpEx."?emission_id=".$emission_id) . '">', '</a>');
...
$template->set_filenames(array(
	'body' => 'site/medias/add_illustration.tpl',
...
$template->assign_vars(array(
				...
				"U_FORM" => append_sid("add_illustration.php?emission_id=".$_GET['emission_id']."&mode=add"),													
				"AJOUT_ILLU" => $lang['add_illustration'], 
