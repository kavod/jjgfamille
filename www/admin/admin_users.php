...
if ( $mode == 'edit' || $mode == 'save' && ( isset($HTTP_POST_VARS['username']) || isset($HTTP_GET_VARS[POST_USERS_URL]) || isset( $HTTP_POST_VARS[POST_USERS_URL]) ) )
{
	...
	if ( ( $mode == 'save' && isset( $HTTP_POST_VARS['submit'] ) ) || isset( $HTTP_POST_VARS['avatargallery'] ) || isset( $HTTP_POST_VARS['submitavatar'] ) || isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		if( $HTTP_POST_VARS['deleteuser'] && ( $userdata['user_id'] != $user_id ) )
		{
			/*
			//
			//  Déplacé dans includes/functions.php
			//
		...
		$birth_day = $HTTP_POST_VARS['birth_day'];
		$birth_month = $HTTP_POST_VARS['birth_month'];
		$birth_year = $HTTP_POST_VARS['birth_year'];
	...
	if( isset( $HTTP_POST_VARS['submit'] ) )
	{
		...
		{
			if (!unlink("../images/photos/photo_".$user_id.".gif"))
				if (unlink("../images/photos/photo_".$user_id.".jpg"))
					unlink("../images/photos/photo_".$user_id.".png");
			$photo_sql = ", photo = 'N' ";
		}
		...
		if( isset($HTTP_POST_VARS['avatardel']) )
		{
			...
			{
				if( @file_exists(@phpbb_realpath($phpbb_root_path . $board_config['avatar_path'] . "/" . $this_userdata['user_avatar'])) )
				{
					@unlink($phpbb_root_path . $board_config['avatar_path'] . "/" . $this_userdata['user_avatar']);
				}
			}
		...
		if( !$error )
		{
			$sql = "UPDATE " . USERS_TABLE . "
				SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) . "', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", $aim) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_attachsig = $attachsig, user_sig_bbcode_uid = '$signature_bbcode_uid', user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowavatar = $user_allowavatar, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_allow_pm = $user_allowpm, user_notify_pm = $notifypm, user_popup_pm = $popuppm, user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_active = $user_status, user_rank = $user_rank" . $avatar_sql . ", birth_day = " . $birth_day . ", birth_month = " . $birth_month . ", birth_year = " . $birth_year . $photo_sql .
				" WHERE user_id = $user_id";
	...
	else if( !isset( $HTTP_POST_VARS['submit'] ) && $mode != 'save' && !isset( $HTTP_POST_VARS['avatargallery'] ) && !isset( $HTTP_POST_VARS['submitavatar'] ) && !isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		if ($this_userdata['birth_day']!=0) $birth_day = $this_userdata['birth_day'];
		if ($this_userdata['birth_month']!=0) $birth_month = $this_userdata['birth_month'];
		if ($this_userdata['birth_year']!=1000) $birth_year = $this_userdata['birth_year'];
