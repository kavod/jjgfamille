...
if ($_GET['mode'] == 'add_mascotte')
{
	...
	if ($user_mascotte_upload!= '')
	{
		...
		$mascotte_sql = user_mascotte_upload($error, $error_msg, $user_mascotte_upload, $user_mascotte_name, $user_mascotte_size, $user_mascotte_filetype,'tournees');
		...
		else
		{
			if (!$error)
			{
				logger('Modification de la mascotte Tournées');
...
$template->set_filenames(array(
	'body' => 'site/tournees/edit_mascotte.tpl',
...
if ( $userdata['user_level'] == ADMIN || is_responsable($userdata['user_id'],'mascotte'))
{
	$template->assign_block_vars('switch_edit_mascotte', array(
					...
					"U_UPLOAD" => append_sid($phpbb_root_path . 'tournees/edit_mascotte.php?mode=add_mascotte'),
