...
if ($_GET['mode'] == 'add')
{
	...
	if (!$error)
	{
		$sql_update = "UPDATE tournee_recits SET recit='".$comment."',bbcode_uid='".$bbcode_uid."' WHERE recit_id='".$recit_id."'";
		...
		logger("Modification de l'avis de concert qui date du " . affiche_date($val_concert['date']));
		
		if (!$error)
		{
			logger("Modification du recit N°$recit_id de la tournee N°$tournee_id");
...
//Le recit
$val_recit = select_element("SELECT * FROM tournee_recits WHERE  recit_id= ".$_GET['recit_id']." LIMIT 0,1",'',false);
...
$template->set_filenames(array(
	'body' => 'site/tournees/edit_recit.tpl',
...
$template->assign_vars(array(
				...
				"U_FORM" => append_sid("edit_recit.php?concert_id=".$_GET['concert_id']."&recit_id=".$val_recit['recit_id']."&mode=add"),													
				"AJOUT_RETRAN" => $lang['modif_recit'], 
				"RECIT" => str_replace("<br />","",preg_replace('/\:(([a-z0-9]:)?)' . $val_recit['bbcode_uid'] . '/s', '', $val_recit['recit'])), 

				"L_SUPPRIMER" => $lang['supprimer']."&nbsp;".sprintf($lang['le'],$lang['recit']),
				"U_SUPPRIMER" => append_sid($phpbb_root_path . 'tournees/doedit.php?mode=supp_recit&recit_id='.$val_recit['recit_id'].'&concert_id='.$val_emission['concert_id']),		
