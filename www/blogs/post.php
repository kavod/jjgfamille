 ...
 *                                post.php
 *                            -------------------
 *   begin                : Saturday, Sept 10, 2006
 *   copyright            : (C) 2006 Boris Kavod
 *   email                : 
 *
...
//
$params = array('submit' => 'post', 'mode' => 'mode');
...
$params = array('topic_id' => 'article_id', 'forum_id' => 'forum_id', 'post_id' => 'p');
while( list($var, $param) = @each($params) )
{
	if ( !empty($HTTP_POST_VARS[$param]) || !empty($HTTP_GET_VARS[$param]) )
	{
...
if ($mode == 'move')
{
	...
	$val_article = select_article($topic_id);
	$val_cate = select_cate($val_article['forum_id']);
	move_article($HTTP_POST_VARS['topic_id_list'],$topic_id,$HTTP_POST_VARS['new_forum'],$val_cate['cate_id'],$confirm,$val_cate['user_id']);
	exit();
}
...
switch( $mode )
{
	...
	//case 'quote':
		...
		// Pour reconnaitre un message blog d'un message forum, nous faisons établierons que $forum_id = - $cate_id du blog
		$val_blogger = select_element("SELECT `forum_id`, `topic_poster` FROM `blogs_articles` WHERE `topic_id` = '$topic_id'",true,'Catégorie introuvable');
		$forum_id = $val_blogger['forum_id'];
		$val_blog = select_blog($val_blogger['topic_poster']);
		$val_blogger = select_blogger($val_blogger['topic_poster']);
		break;
	case 'editpost':
		$is_auth_type = 'auth_edit';
		// Pour reconnaitre un message blog d'un message forum, nous faisons établierons que $forum_id = - $cate_id du blog
		$val_blogger = select_element("SELECT a.`forum_id` FROM `blogs_articles` a, `blogs_coms` c WHERE a.`topic_id` = c.`topic_id` AND c.`post_id` = '$post_id'",true,'Catégorie introuvable');
		$forum_id = $val_blogger['forum_id'];
		break;
	case 'delete':
	//case 'poll_delete':
...
switch ( $mode )
{
	...
	//case 'vote':
		...
		$sql = "SELECT c.*, a.topic_status, a.topic_title, b.`enable`, b.`readonly` 
			FROM blogs_cate c, blogs_articles a, blogs_blogs b
			WHERE a.topic_id = $topic_id
				AND c.cate_id = a.forum_id
	...
	/*case 'poll_delete':*/
		...
		$from_sql = ( !$submit ) ? ", blogs_coms_text pt, " . USERS_TABLE . " u" : '';
		$where_sql = ( !$submit ) ? "AND pt.post_id = p.post_id AND u.user_id = p.poster_id" : '';

		$sql = "SELECT f.*, t.topic_id, t.topic_status, t.topic_first_post_id, p.post_id, p.poster_id" . $select_sql . " 
			FROM blogs_coms p, blogs_articles t, blogs_cate f" . $from_sql . " 
			WHERE p.post_id = $post_id 
				AND t.topic_id = p.topic_id 
				AND f.cate_id = p.forum_id
				$where_sql";
		break;
...
if ( $result = $db->sql_query($sql) )
{
	...
	if (empty($post_info['forum_id']) || $post_info['forum_id'] == '')
		$post_info['forum_id'] = $post_info['cate_id'];
	
	/*
	 * Qu'est-ce que j'ai foutu ca ????
	$user_id = $post_info['user_id'];
	$username = $post_info['username'];
	*/
	
	$forum_id = $post_info['cate_id'];
	$forum_name = $post_info['cate_name'];

	$is_auth = auth(AUTH_ALL, (0-$forum_id), $userdata, $post_info);
	...
	if ( $mode == 'editpost' || $mode == 'delete' || $mode == 'poll_delete' )
	{
		...
		if ( $post_info['poster_id'] != $userdata['user_id'] && !$is_auth['auth_mod'] )
		{
			...
			$message .= '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . append_sid("?article_id=$topic_id") . '">', '</a>');

			message_die(GENERAL_MESSAGE, $message);
		}
		else if ( !$post_data['last_post'] && !$is_auth['auth_mod'] && ( $mode == 'delete' || $delete ) )
		{
			echo $is_auth['auth_mod'];
	...
	else
	{
		if ( $mode == 'reply' )
		{
			if ($post_info['readonly'] != 'N')
				message_die(GENERAL_MESSAGE,$lang['Blog_readonly']);
		}
		if ( $mode == 'quote' )
		{
			$topic_id = $post_info['topic_id'];
...
{
	message_die(GENERAL_MESSAGE, $lang['No_such_post']."<br />$sql");
}
...
if ( !$is_auth[$is_auth_type] )
{
	...
	{
		case 'newtopic':
			$redirect = "mode=newtopic&cate_id=" . $forum_id;
			break;
		case 'reply':
		case 'topicreview':
			$redirect = "mode=reply&article_id=" . $topic_id;
			break;
		case 'quote':
		case 'editpost':
			$redirect = "mode=quote&post_id=" . $post_id;
			break;
	}

	redirect(append_sid($phpbb_root_path . "forum/login.$phpEx?redirect=" . $phpbb_root_path . "blogs/post.php&" . $redirect, true));
}
...
if ( ( $delete || $poll_delete || $mode == 'delete' ) && !$confirm )
{
	...
	$template->assign_vars(array(
		...
		'S_CONFIRM_ACTION' => append_sid("post.$phpEx"),
...
else*/ if ( $submit || $confirm )
{
	...
	switch ( $mode )
	{
		...
		case 'reply':
			...
			if ( $error_msg == '' )
			{
				...
				$neg_forum_id = -$forum_id;
				submit_post($mode, $post_data, $return_message, $return_meta, $neg_forum_id, $topic_id, $post_id, $poll_id, $topic_type, $bbcode_on, $html_on, $smilies_on, $attach_sig, $bbcode_uid, str_replace("\'", "''", $username), str_replace("\'", "''", $subject), str_replace("\'", "''", $message), str_replace("\'", "''", $poll_title), $poll_options, $poll_length, $max_vote, $hide_vote, $undo_vote);
			}
			break;

		case 'delete':
		case 'poll_delete':
			delete_com($mode, $post_data, $return_message, $return_meta, $forum_id, $topic_id, $post_id, $poll_id);
			break;
	}
	...
	if ( $error_msg == '' )
	{
		if ( $mode != 'editpost' )
		{
			...
			update_post_stats($mode, $post_data, $neg_forum_id, $topic_id, $post_id, $user_id);
		}
		// Notification
		if ($val_blog['notification']=='Y')
		{
			include($phpbb_root_path . 'includes/emailer.'.$phpEx);
			$emailer = new emailer($board_config['smtp_delivery']);
			$emailer->from($board_config['board_email']);
			$emailer->replyto($board_config['board_email']);
			$emailer->use_template('blog_notify', $user_lang);
			$emailer->set_subject($lang['Topic_reply_notification']); 
			//$emailer->msg = preg_replace('#[ ]?{USERNAME}#', '', $emailer->msg);
			
			$script_name = preg_replace('/^\/?(.*?)\/?$/', '\1', trim('blogs/'));
			...
			$emailer->assign_vars(array(
						...
						'U_TOPIC' => $server_protocol . $server_name . $server_port . $script_name . "?article_id=" . $topic_id . "&nid=3#$post_id",
						'USERNAME' => $val_blogger['username'],
						)
					);
			$emailer->email_address($val_blogger['user_email']);
			$emailer->send();
			$emailer->reset();
		}
		// fin Notification
...
//
$topic_type_toggle = '';
$is_auth['auth_sticky'] = false;
$is_auth['auth_announce'] = false;
...
switch( $mode )
{
	case 'newtopic':
		...
		$hidden_form_fields .= '<input type="hidden" name="forum_id" value="' . $forum_id . '" />';
		break;

	case 'reply':
		$page_title = $lang['Post_a_reply'];
		$hidden_form_fields .= '<input type="hidden" name="article_id" value="' . $topic_id . '" />';
		break;

	case 'editpost':
		$page_title = $lang['Edit_Post'];
		$hidden_form_fields .= '<input type="hidden" name="p" value="' . $post_id . '" />';
		break;
}
...
$template->assign_vars(array(
	...
	'U_VIEW_FORUM' => append_sid($phpbb_root_path . "blogs/?cate_id=$forum_id"))
);
...
$template->assign_vars(array(
	...
	'S_TOPIC_ID' => $topic_id, 
	'S_POST_ACTION' => append_sid("post.php"),
